...
foreach ($email in $users)
{
    ...
    $DN="Gast " + $name
    $OM=$email
    New-MsolUser -UserPrincipalName $UPN -DisplayName $DN -FirstName "Guest" -LastName $name -AlternateEmailAddresses $email -Password $password -UserType "Guest" -ForceChangePassword 1
